...
class CreateDlGoodsSkusTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('dl_goods_skus', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedBigInteger('goods_downloads_id')->comment('下载商品ID');
            ...
            $table->timestamp('created')->nullable()->commet('sku创建日期 时间格式：yyyy-MM-dd HH:mm:ss');
            $table->timestamp('modified')->nullable()->commet('sku最后修改日期 时间格式：yyyy-MM-dd HH:mm:ss');
