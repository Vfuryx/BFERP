...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    rowProCName: function rowProCName(_ref) {
      var row = _ref.row,
    ...
    togglePro: function togglePro(row) {
      this.compIndex = '';
      row.productspecs.map(function (item) {
        item['compDtl'] = {
          purchase_quantity: '',
          shops_id: '',
          suppliers_id: '',
    ...
    delPurDtl: function delPurDtl(row, e, index) {
      ...
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        _this5.purchaseDetailVal.splice(index, 1);
        _this5.$message({
          message: '删除成功',
          type: 'success'
        });
    ...
    confirmEditPur: function confirmEditPur(id) {
      ...
      if (this.chgPurchaseVal.purchase_details.length == 0) {
        ...
        this.$patch(this.purchasesUrl + '/' + id, this.chgPurchaseVal).then(function () {
          console.log(_this7.chgPurchaseVal);
          _this7.$message({
            message: '修改成功',
            type: 'success'
          });
          _this7.chgPurchase = false;
          _this7.refresh();
        }, function (err) {
          if (err.response) {
            var arr = err.response.data.errors;
    ...
    addChgPurDtl: function addChgPurDtl() {
      ...
      this.$fetch(this.warehouseUrl + '/' + id).then(function (res) {
        _this8.proVal.push(res);
        _this8.specVal.push(res.pro_specs);
      }, function (err) {});
    },
    ...
    searchProClick: function searchProClick() {
      ...
      this.$fetch(this.urls.goods, this.searchPro).then(function (res) {
        _this10.proDtlData = res.data;
        res.data[0].productspecs.map(function (item) {
          item['compDtl'] = {
            purchase_quantity: '',
            shops_id: '',
            suppliers_id: '',
        ...
        _this10.specData = res.data[0].productspecs;
        _this10.combData = res.data[0].productspecs[0].combinations;
      }, function (err) {
        if (err.response) {
          var arr = err.response.data.errors;
          var arr1 = [];
    ...
    [
      ...
      _c(
        ...
        [
          ...
          _vm._v(" "),
          _c("el-button", { attrs: { type: "text" } }, [_vm._v("sku列表")]),
          _vm._v(" "),
          _c(
            "el-table",
            {
          ...
          _c(
            "el-table",
            {
              attrs: {
                ...
                type: "index",
                height: "160",
                "row-class-name": _vm.rowProCName
              },
                ...
                scopedSlots: _vm._u([
                  {
                    ...
                    fn: function(scope) {
                      return [
                        item.editAble
                          ? _c("span", [
                              _vm.compIndex == "index" + scope.$index
                                ? _c("span", [
                                    item.type == "number"
                                      ? _c(
                                          "span",
