...
## 修改产品规格 [PATCH /api/productspecs/:id]
...
+ Parameters
    + product_specs_id: (integer, required) - 产品规格id
    + com_pro_specs_id: (integer, required) - 组合产品规格id
    + count: (integer, required) - 组合件数
    + status: (integer, optional) - 状态(0:停用，1:启用)
        + Default: 1
