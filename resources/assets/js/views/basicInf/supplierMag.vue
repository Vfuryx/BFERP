...
<template>
    <div>
        <v-tabs :table-key="tableKey" :url="url" @edit="edit" ref="tabs" :title="title" :rule-form="ruleForm" :rules="rules" :add-arr="addArr" :halfForm="halfForm"></v-tabs>
    </div>
</template>
<script>
  export default {
    data() {
      return {
        ...
        rules: [{
          name: [
            {required: true, message: '请输入供应厂名称', trigger: 'blur'},
          ],
          company: [
    ...
    methods:{
      ...
      edit(row){
        let obj = {
          ...
          is_scan: row.is_scan,
          status: row.status,
          auto_valuationy: row.auto_valuationy
        };
