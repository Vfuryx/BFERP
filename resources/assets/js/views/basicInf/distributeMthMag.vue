<template>
    <div>
        <!--数据-->
        <el-table
                :data="distType" fit highlight-current-row
                type="index"
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                        <span v-if="changeIndex=='index'+scope.$index">
                            <el-input size="small" v-model="scope.row.name" placeholder="配送方式" @change="handleEdit"></el-input>
                        </span>
                    <span v-else>
                            {{scope.row.name}}
                        </span>
                ...
                <template slot-scope="scope">
                        <span v-if="changeIndex=='index'+scope.$index">
                            <el-select v-model="scope.row.status" placeholder="状态" @change="handleEdit">
                                <el-option v-for="item in statusInput" :key="item.value" :label="item.label" :value="item.value"></el-option>
                            </el-select>
                        </span>
                    <span v-else>
        ...
        <!--添加-->
        <el-dialog title="新增配送方式" :visible.sync="showAdd">
  ...
  export default {
    ...
    methods:{
      ...
      handleCurrentChange(val){
        this.$fetch('/distmets?page=' + val).then((res) => {
          this.distType = res.data;
          let pg = res.meta.pagination;
          this.pagination.current_page = pg.current_page;
        }, (err) => {
      ...
      delMore(){
        if (this.delArr.length === 0) {
          ...
          this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
            ...
            this.$del("/distmets", {ids: this.delArr})
              .then(() => {
                this.$message({
                  message: '删除成功',
                  type: 'success'
