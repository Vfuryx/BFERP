                ...
                <el-form-item v-for="(item,index) in addArr" :key="index" :label="item.label" :prop="item.prop">
                    <span v-if="item.type=='text'">
                        <span v-if="item.inProp">
                            <el-input v-model="ruleForm[item.prop][item.inProp]" :placeholder="item.holder" :disabled="editSign?item.editChgAble:item.addChgAble"></el-input>
                        </span>
                        <span v-else>
                          <el-input v-model="ruleForm[item.prop]" :placeholder="item.holder" :disabled="editSign?item.editChgAble:item.addChgAble"></el-input>
                        </span>
                    </span>
                    <span v-else-if="item.type=='tel'">
                       <el-input type="tel" v-model="ruleForm[item.prop]" :placeholder="item.holder" :disabled="editSign?item.editChgAble:item.addChgAble"></el-input>
                    </span>
                    <span v-else-if="item.type=='number'">
                       <el-input type="number" v-model="ruleForm[item.prop]" :placeholder="item.holder" :disabled="editSign?item.editChgAble:item.addChgAble"></el-input>
                    </span>
                    <span v-else-if="item.type=='url'">
                       <el-input type="url" v-model="ruleForm[item.prop]" :placeholder="item.holder" :disabled="editSign?item.editChgAble:item.addChgAble"></el-input>
                    </span>
                    ...
                    <span v-else-if="item.type=='textarea'">
                         <el-input type="textarea" v-model="ruleForm[item.prop]" :placehoder="item.holder"></el-input>
                    </span>
                    ...
                    <span v-if="item.type=='text'">
                      <!-- <el-input v-model="ruleForm[item.prop]" :placeholder="item.holder" :disabled="editSign?item.editChgAble:item.addChgAble"></el-input>-->
                       ...
                       <el-color-picker v-model="ruleForm[item.prop]"></el-color-picker>
                        ...
                        <span v-else-if="item.type=='select_stu'">
                        <el-select v-model="ruleForm[item.prop]" :placeholderer="item.holder" :disabled="editSign?item.editChgAble:item.addChgAble">
                        <el-option label="停用" value="0"></el-option>
                        <el-option label="启用" value="1"></el-option>
                    </el-select>
                    </span>
...
<script>
  ...
  export default {
    props: ['visibleAdd','title', 'ruleForm', 'rules', 'addArr','submitData','url','newRef','halfForm','selects','leftTab','moreForms','onlyInputs','hasSelect','editSign'],
    data() {
      return {
        showAdd: false,
    ...
    methods: {
     ...
     /* getVal(val){
        // this.defVal = val;
        this.defArr=[{id:'0',name:'停用'},{id:'1',name:'启用'}];
      }*/
    },
    mounted(){
      // this.getVal();
